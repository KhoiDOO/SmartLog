...
class SmartSupportVectorMachine(SmartTraining):
    def __init__(self, 
                 number_class=2, 
                 number_label=1, 
                 number_fold=5, 
    ...
    def smartfit(self):
        count = 0
        for x in self.params_dict["kernel"]:
            for i in self.params_dict["degree"]:
                for j in self.params_dict["gamma"]:
                    for k in self.params_dict["class_weight"]:
                            print("Traning Case: {}".format(count))
                            self.training(x, i, j, k, 5)
